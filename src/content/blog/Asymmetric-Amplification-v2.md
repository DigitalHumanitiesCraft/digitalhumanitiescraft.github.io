...
Two methodological instruments from my own practice contribute to this capacity, and they should not be conflated. Epistemic infrastructure is the broader arrangement that makes agent-supported knowledge production reconstructable, from versioned data states to attributed judgement. Promptotyping is one specific method for building and revising such infrastructure at project level, and it deserves a conceptual sketch here, because the method is defined by its structure and travels across projects.

Promptotyping is an iterative, knowledge-driven method for developing project-specific digital research artefacts from structured research data and maintained project knowledge, through Context Engineering and Agentic Engineering. Its organising structure is an evolving, versioned project knowledge base. Distillation, the writing of this knowledge base, is pragmatic modelling rather than summarisation; it decides which distinctions of the research material an artefact has to operationalise, and this is conceptual work that no agent performs on anyone's behalf. The knowledge base guides bounded agentic implementation, implementation makes the recorded assumptions actionable and exposes their consequences, the resulting artefacts are examined, and consequential findings are written back into the knowledge base. The compounding loop of the previous sections is this method's core, deliberately institutionalised at project scale.

The method's unit of progress is the promptotype, an identifiable, bounded, accepted iteration state that connects a documented state of project knowledge, a referenced state of the research data, a resulting artefact, and the recorded grounds of acceptance. Acceptance is purpose-specific; a promptotype is accepted for a stated scholarly use, with the differentiated checking described above behind it. The method is the subject of a separate scholarly paper, where its cases and limits are documented; here it matters as evidence that engagement on scholarly terms is buildable rather than aspirational.
